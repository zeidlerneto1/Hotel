...
class AtualizarClienteWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Atualizar Cadastro de Cliente')
    ...
    def initUI(self):
        ...
        # Botão para atualizar dados do cliente
        self.btn_atualizar = QPushButton('Atualizar')
        self.btn_atualizar.clicked.connect(self.atualizar_cliente)
        self.layout_dados.addWidget(self.btn_atualizar)
    ...
    def atualizar_cliente(self):
        ...
        if not nome_cliente:
            QMessageBox.warning(self, 'Atualizar Cliente', 'Por favor, selecione um cliente da lista.')
            return

        novo_endereco = self.input_endereco.text()
        novo_telefone = self.input_telefone.text()
        novo_email = self.input_email.text()
        novo_documento = self.input_documento.text()
        ...
        data = {
            ...
            'email': novo_email,
            'documento_RG_cliente': novo_documento
        }
        ...
        try:
            ...
            if response.status_code == 200:
                QMessageBox.information(self, 'Atualizar Cliente', 'Cliente atualizado com sucesso!')
                self.limpar_campos()
                self.carregar_lista_clientes()  # Atualizar a lista de clientes após a atualização
            else:
                QMessageBox.warning(self, 'Atualizar Cliente', f'Erro ao atualizar cliente: {response.status_code}')

        except requests.exceptions.RequestException as e:
            QMessageBox.critical(self, 'Erro de Conexão', f'Erro ao conectar à API: {str(e)}')

    def buscar_id_por_nome(self, nome_cliente):
        try:
            url = 'http://127.0.0.1:5000/api/clientes'
            response = requests.get(url)
            clientes = response.json()
